...
class DateTimeRangePickerViewModel(private val timeFormatter: TimeFormatter) {
  ...
  internal val minDate: Date by lazy { DateTime.now(DateTimeZone.forTimeZone(timeZone)).minusYears(1).toDate() }
  ...
  companion object {
    const val KEY_START_TIME_IN_MILLIS = "startTimeInMillis"
    const val KEY_END_TIME_IN_MILLIS = "endTimeInMillis"
